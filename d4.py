inp = """nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
...
n = len(alph)
for i in inp.split('\n'):
    stuff,other = i.rsplit('-',1)
    num = other.split('[')[0]
    chk = other.split('[')[1].replace(']','')
    a = stuff.replace('-',' ')
    """ com = collections.defaultdict(int)

    for l in a:
        com[l] += 1
    stu = sorted(com.items(), key=lambda x:x[0])
    print(stu)
    fin = sorted(stu, key=lambda x:x[1], reverse=True)
    print(fin)
    print(map(lambda x:x[0], fin[:5]))
    print(chk)
    if ''.join(map(lambda x:x[0], fin[:5])) == chk:
        count += int(num)
    """
    res = ''
    for c in a:
